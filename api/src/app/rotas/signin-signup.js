...
module.exports = (app) => {
    ...
    app.post('/registro-cliente',  async (req, res) => {
        ...
        if ( await Client.findOne({email})) return res.status(400).json({ error: 'Email já cadastrado'});
        ...
        try {
           ...
           return res.status(512).json({ error: 'Falha ao tentar realizar o cadastro' });
        } 
    });
    ...
    app.post('/registro-empresa',  async (req, res) => {
        ...
        if ( await Empresa.findOne({ email })) return res.status(400).json({ error: 'Email já cadastrado'});
        if ( await Empresa.findOne({cnpj})) return res.status(400).json({ error: 'CNPJ já cadastrado'});

        try {

            await Empresa.create(req.body).then((retorno) => {
                retorno.senha = undefined;
                return res.json(retorno);
            });
    ...
    app.post('/autenticacao-empresa', async (req, res) => {
        ...
        const { email, senha } = req.body;
        const usuario = await Empresa.findOne({ email }).select('+senha');
